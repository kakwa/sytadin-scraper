...
usage: 
   ...
   sytadin-scraper -s '<start node>' -e '<end node>' [-v '<intermediate node>']

Command line utility to query www.sytadin.fr

args:
    -h: display this help
    -S <node name>: search the name matching '.*<node name>.*' in available nodes (case insensitive)
...
> sytadin-scraper -s 'Le Petit Clamart (N118xA86)' -e 'Evry (A6xN104)' -v 'Janvry (A10xN104)'
